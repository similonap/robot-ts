...
async function main() {
    const robot = circuit_crawler_1.game.getRobot("Tester");
    if (!robot)
    ...
    if (res1.success)
        console.log("✅ Success: Opened normal door");
    else
        circuit_crawler_1.game.fail("Failed to open normal door: " + res1.message);
    ...
    if (!res2Fail.success)
        console.log("✅ Verified: Cannot open password door without password");
    else
        circuit_crawler_1.game.fail("Should not open password door without password!");
    // Try with wrong password
    const res2Wrong = await robot.openDoor("wrong");
    if (!res2Wrong.success)
        console.log("✅ Verified: Cannot open password door with wrong password");
    else
        circuit_crawler_1.game.fail("Should not open password door with wrong password!");
    // Try with correct password
    console.log("Attempting to open password door with '1234'...");
    const res2 = await robot.openDoor("1234");
    if (res2.success)
        console.log("✅ Success: Opened password door");
    else
        circuit_crawler_1.game.fail("Failed to open password door: " + res2.message);
    ...
    if (key && key.id === 'key-card')
        console.log("✅ Success: Picked up Key Card");
    else
        circuit_crawler_1.game.fail("Failed to pick up key card");
    ...
    if (!res3Fail.success)
        console.log("✅ Verified: Cannot open item door without providing item");
    else
        circuit_crawler_1.game.fail("Should not open item door without providing item!");
    // Open with item
    console.log("Attempting to open item door WITH Key Card...");
    const res3 = await robot.openDoor(key);
    if (res3.success)
        console.log("✅ Success: Opened item door");
    else
        circuit_crawler_1.game.fail("Failed to open item door: " + res3.message);
    await robot.moveForward(); // Move into door (6,1)
    circuit_crawler_1.game.win("All door tests passed!");
}

...
exports.RobotController = exports.HealthDepletedError = exports.CrashError = exports.CancelError = void 0;
...
class CrashError extends Error {
    constructor(message = 'Robot crashed') {
        super(message);
        this.name = 'CrashError';
    }
}
exports.CrashError = CrashError;
class HealthDepletedError extends Error {
    constructor(message = 'Robot health depleted') {
        super(message);
        this.name = 'HealthDepletedError';
    }
}
exports.HealthDepletedError = HealthDepletedError;
// RobotState imported from types.ts
class RobotController {
    constructor(initialState, walls, world, onUpdate, signal, items = [], doors = [], checkGameOver = () => true) {
        this.delayMs = 500;
    ...
    isWall(x, y) {
        if (y < 0 || y >= this.walls.length || x < 0 || x >= this.walls[0].length)
            return true;
        ...
        if (door) {
            ...
            if (isOpen)
                return false; // Open door = Walkable
            return true;
        }
    ...
    async moveForward() {
        ...
        const hiddenItemsHere = this.items.filter(item => item.position.x === newX &&
    ...
    async pickup() {
        ...
        const { x, y } = this.robotState.position;
        // Check for items
        const itemAtPos = this.items.find(item => item.position.x === x &&
            item.position.y === y &&
    ...
    async echo() {
        ...
        while (true) {
            ...
            const item = this.items.find(i => i.position.x === cx &&
                i.position.y === cy &&
    ...
    async scan() {
        ...
        const itemAtPos = this.items.find(item => item.position.x === scanX &&
